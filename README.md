...
- Free-tier framing for one mortgage and one GIC, with Plus prompts for additional mortgages and GICs.
- Profile tab with notification toggles, Data Sources, V1 scope notes, and Plus positioning.
- Plus-framed prototype interaction for dragging institution watchlist rows into a custom order.

## Data Note

The Bank of Canada policy-rate and prime-rate references are hard-coded from recent public sources for the prototype. Institution mortgage, savings, and GIC rows remain sample placeholders until live source ingestion is added.
